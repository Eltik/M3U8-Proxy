...
import { proxy, proxyTs, web_server_url } from "./M3U8Proxy";
...
function getHandler(options, proxy) {
    ...
    const hasRequiredHeaders = function(headers) {
        return !corsAnywhere.requireHeader || (corsAnywhere.requireHeader as any).some(function(headerName) {
    ...
    return function(req, res) {
        ...
        function isValidHostName(hostname) {
            ...
            return !!(
                regexp.test(hostname) || net.isIPv4(hostname) || net.isIPv6(hostname)
            );
        }
